...
```
<html data-lang-active="vi|en">
  [data-lang="vi"] blocks visible when vi
  [data-lang="en"] blocks hidden via CSS
```

Toggle button `#lang-toggle` shows **EN** when current is VI (switch to EN).
